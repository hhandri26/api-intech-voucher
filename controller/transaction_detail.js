...
exports.voucher = function(req, res) {
    ...
    var status = "'DONE'";
    connection.query('SELECT * , DATE_FORMAT(created_at, "%d/%m/%Y") as date, FORMAT(price, 0) as harga FROM tbl_transaction_detail  where status <> '+status+' and id_user = ? order by created_at DESC',[id], function (error, rows, fields){
        if(error){
            console.log(error)
        } else{
...
exports.voucherAll = function(req, res) {
    var id = req.params.id;
    connection.query('SELECT a.*,DATE_FORMAT(a.created_at, "%d/%m/%Y") as date,IF(a.lokasi IS NULL or a.lokasi = "", "empty", a.lokasi) as lok,FORMAT(a.price, 0) as harga, b.username as reseller FROM tbl_transaction_detail as a LEFT JOIN users as b ON a.id_user = b.id order by created_at DESC LIMIT 100', function (error, rows, fields){
        if(error){
            console.log(error)
        } else{
            response.ok(rows, res)
